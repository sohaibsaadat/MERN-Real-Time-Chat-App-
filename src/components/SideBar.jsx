...
{filteredUsers.map((user,index)=>(
 <div onClick={()=>{setSelectedUser(user);setUnseenMsg(prev=>({...prev,[user._id]:0}))}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rouded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id &&'bg-[#282142]/50'}`}>
...
<p className='text-xs'>{user.fullName}</p>
{
   onlineUsers.includes(user._id)  ? <span className='text-green-400 text-xs'>Online</span>
    ...
    {
        unseenMsg[user._id] > 0 && <p className='absolute top-4 right-4 text-xs h-5 w-5 flex justify-center items-center rounded-full bg-violet-500/50'>{ unseenMsg[user._id] }</p>
    }
 </div>
))}
      </div>
